...
## 13. C Types Diagnosed But Not Represented (`long double`, `va_list`, `enum`)
...
## 14. Passing a Struct By Value to a C Function Is Unsupported
...
## 15. No CRT Startup Objects Are Linked — `palan-codegen` Supplies ELF/libc Glue Itself
...
## 16. `select`/`pselect` Remain Unusable — `fd_set`'s `sizeof`-Sized Array Field
...
## 17. Pointer-Bottomed C Typedefs Are Not Registered As Type Names
...
## 18. `unistd.h`'s `syscall()` Wrapper Is Shadowed by the `syscall` Reserved Word
...
## 19. C11 Anonymous Members Without a Member Name Fail to Parse
...
## 20. Function Definitions With an Unsupported Parameter Form Are Silently Dropped
