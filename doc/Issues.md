...
## 23. Cinclude Macro Constants Are Resolved as an SA Symbol, Not Folded at Parse Time

**Summary:** A cinclude'd C macro constant (`SYS_write`, `S_IFMT`, etc.) is fully folded to a concrete literal by c2ast at gen-ast time (`CParser::resolveConstValue`, arbitrary indirection via `CPreprocessor::expandObjectMacroBody`) and stored by name in the cinclude statement's `constants` list. But a *reference* to that name elsewhere in Palan source (e.g. `= SYS_write`) is left as a plain `id` node by gen-ast — substitution only happens later, in SA (`sa_expression`'s id branch consulting `constDecls_`, itself populated from `constants` during `sa_cinclude`). Since the macro constant is fundamentally a C-preprocessor textual-substitution concept, not a Palan symbol with its own scope/type semantics, folding the reference at parse time (gen-ast) instead of deferring to an SA symbol table would let any value built from one (an exported constant, a future syscall number, etc.) be self-contained by the time `ast.json` is written — no dependency on the referencing module's own SA-time symbol table, and no cross-module import gap. Doing this inside the GLR parser's own reduction actions needs care: `PlnParser.yy`'s cinclude action (`:181-206`) already spawns `palan-c2ast` from a semantic action, so a speculative/discarded GLR branch already tolerates wasted subprocess work, but a growing macro-name-to-literal table consulted by *later* reductions to decide whether to substitute is riskier shared mutable state than that — unlike accumulation into `ast["export"]` (which existing `do_export` actions already do), an id-vs-literal decision changes how a later token is typed, so a decision made by a branch that later gets discarded could diverge from the branch that survives. Likely needs a dedicated post-parse AST-walking pass in gen-ast's driver code (after the single surviving parse tree is built) rather than a grammar action, to stay out of that GLR-purity concern. Native Palan `const Name = expr;` chains are a separate, legitimately SA-owned concept (see item 10) and should stay resolved by SA regardless of this change. Surfaced while designing v0.1.32's syscall-number handling (`localtickets/iteration-2026-09-19-v0132-syscall.md`); that iteration sidesteps the gap by restricting the syscall number to a literal integer rather than taking on this redesign.

---

## 24. `unistd.h`'s `syscall()` Wrapper Is Shadowed by the `syscall` Reserved Word
